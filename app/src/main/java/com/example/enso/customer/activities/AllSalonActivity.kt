...
class AllSalonActivity : AppCompatActivity() {
    ...
    private fun fetchSalons() {
        ...
        // WHY: Pending salons must remain hidden to protect the marketplace from unverified or low-quality listings.
        // WHAT: We filter the query to only return salons where 'status' is 'active' (meaning they were approved by an admin).
        // HOW: Firebase server-side filtering (orderByChild + equalTo) improves marketplace safety by ensuring only vetted businesses reach the customer's screen.
        val approvedQuery = database.orderByChild("status").equalTo("active")

        approvedQuery.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    Toast.makeText(this@AllSalonActivity, "No active salons found", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@AllSalonActivity, "Failed to load data: ${error.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }
}
