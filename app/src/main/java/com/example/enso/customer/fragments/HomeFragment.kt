...
import com.example.enso.customer.activities.AllSalonActivity
...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private fun fetchSalons() {
        ...
        // WHY: Pending salons must remain hidden to ensure customers only interact with verified and vetted businesses.
        // WHAT: We use Firebase's orderByChild and equalTo to filter only salons with 'active' (approved) status.
        // HOW: Firebase filtering improves marketplace safety by preventing unapproved salons from appearing in client search results.
        val approvedQuery = database.orderByChild("status").equalTo("active").limitToFirst(2)
